...
import type { SocialProviderKey } from '@/types/api';
...
> = {
  ...
  tiktok: {
    container: 'bg-black ring-1 ring-white/10',
    icon: 'text-white',
  },
};
...
export function SocialProviderIcon({
  ...
  return (
    <span
      ...
      aria-hidden
    >
      {provider === 'facebook' && <FacebookIcon className={styles.icon} />}
      {provider === 'instagram' && <InstagramIcon className={styles.icon} />}
      {provider === 'tiktok' && <TikTokIcon className={styles.icon} />}
    </span>
  );
}
